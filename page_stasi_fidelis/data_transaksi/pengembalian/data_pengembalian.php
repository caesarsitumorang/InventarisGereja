...
if (isset($_POST['ajax'])) {
    ...
    // Default lokasi Fidelis
    $lokasi_filter = isset($_POST['lokasi_filter']) ? trim($_POST['lokasi_filter']) : 'Stasi St. Fidelis (Karo Simalem)';
    ...
    if (!empty($search)) {
        ...
        $conditions[] = "(nama_barang LIKE '%$search%' 
                          ...
                          OR no_pengembalian LIKE '%$search%' 
                          OR pengembali LIKE '%$search%')";
    }
    ...
    // Hitung total
    $total_records_query = "SELECT COUNT(*) as count FROM pengembalian $where";
...
<div class="container">
    <div class="page-header">
        <h2>Pengembalian - Lokasi: Stasi St. Fidelis (Karo Simalem)</h2>
    </div>

    <div class="toolbar">
        <div class="left-tools">
            <div id="fidelisButtons" style="display:none;">
...
<!-- Tambahkan Modal Download -->
<div id="locationModal" class="modal" style="display:none; position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(0,0,0,0.5); align-items:center; justify-content:center; z-index:1000;">
    <div class="modal-content" style="background:#fff; padding:20px; border-radius:10px; width:400px; max-width:90%;">
        ...
        <div style="margin-top:15px; text-align:right;">
            ...
            <button class="btn btn-success" onclick="downloadPDF('Stasi St. Fidelis (Karo Simalem)')">Download PDF</button>
        </div>
    </div>
</div>
...
function loadData(page = 1) {
    ...
    // Tampilkan tombol hanya jika Fidelis
    if (lokasi_filter === "Stasi St. Fidelis (Karo Simalem)") {
        document.getElementById('fidelisButtons').style.display = 'inline-block';
    } else {
        document.getElementById('fidelisButtons').style.display = 'none';
    }
    ...
    fetch('index_stasi_fidelis.php?page_stasi_fidelis=data_transaksi/pengembalian/data_pengembalian', {
        method: 'POST',
...
document.addEventListener('DOMContentLoaded', function() {
    // Set default filter ke Fidelis
    document.getElementById('filterSelect').value = "Stasi St. Fidelis (Karo Simalem)";
    loadData(1);
...
function downloadPDF(lokasi) {
    ...
    const url = 'index_stasi_fidelis.php?page_stasi_fidelis=data_transaksi/pengembalian/cetak_pengembalian&lokasi=' + 
...
function showAddForm() {
    window.location.href = 'index_stasi_fidelis.php?page_stasi_fidelis=data_transaksi/pengembalian/tambah_pengembalian';
}
