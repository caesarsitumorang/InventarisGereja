...
mysqli_stmt_execute($stmt);
    ...
    echo "<script>alert('Data mutasi berhasil ditambahkan');
          window.location.href='index_stasi_fidelis.php?page_stasi_fidelis=data_transaksi/mutasi/data_mutasi';
          </script>";
}
?>
...
<div class="form-container">
    ...
    <div class="form-group half">
        ...
        <input type="text" name="lokasi_awal" id="lokasi_awal" 
               value="Stasi St. Fidelis (Karo Simalem)" 
...
<div class="form-row">
    ...
    <div class="form-group half">
        ...
        <select id="lokasi_mutasi" name="lokasi_mutasi" required class="form-control" onchange="generateKodeBarangBaru()">
            ...
            <?php 
            $lokasi_mutasi = mysqli_query(
                $koneksi,
                "SELECT nama_lokasi 
                 FROM lokasi 
                 WHERE nama_lokasi NOT LIKE '%Paroki%' 
                 AND nama_lokasi <> 'Stasi St. Fidelis (Karo Simalem)'
                 ORDER BY nama_lokasi ASC"
            );
            while($rowLokasi = mysqli_fetch_assoc($lokasi_mutasi)) { ?>
                <option value="<?= htmlspecialchars($rowLokasi['nama_lokasi']); ?>">
                    <?= htmlspecialchars($rowLokasi['nama_lokasi']); ?>
                </option>
            <?php } ?>
        </select>
    </div>
            ...
            <div class="form-actions">
                ...
                <a href="index_stasi_fidelis.php?page_stasi_fidelis=data_transaksi/mutasi/data_mutasi" class="btn-cancel">Batal</a>
            </div>
        </form>
    </div>
</div>
...
function generateKodeBarangBaru() {
    ...
    if(kategori && lokasi) {
        fetch("page_stasi_fidelis/data_inventaris/agustinus/get_kode_barang.php?kategori="+encodeURIComponent(kategori)+"&lokasi="+encodeURIComponent(lokasi))
        .then(res => res.text())
        .then(data => {
...
// 🔹 Tambahan: otomatis load data saat halaman dibuka
