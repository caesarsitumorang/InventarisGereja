...
if (isset($_POST['ajax'])) {
    ...
    if (!empty($lokasi_filter)) {
        ...
        $conditions[] = "lokasi_mutasi = '$lokasi_escaped'";
    }

    // Search
    if (!empty($search)) {
        $search_escaped = mysqli_real_escape_string($koneksi, $search);
        $conditions[] = "(nama_barang LIKE '%$search_escaped%' 
                          OR kode_barang LIKE '%$search_escaped%' 
                          OR no_mutasi LIKE '%$search_escaped%' 
                          OR keterangan LIKE '%$search_escaped%')";
    }
    ...
    // Hitung total data
    $total_query = "SELECT COUNT(*) AS count FROM mutasi $where";
        ...
        <table class="data-table">
            ...
            <tbody>
                ...
                while ($row = mysqli_fetch_assoc($result)) { ?>
                    <tr>
                        <td><?= $no++; ?></td>
                        <td><?= htmlspecialchars($row['no_mutasi']); ?></td>
                        <td><?= htmlspecialchars(date('d-m-Y', strtotime($row['tanggal_mutasi']))); ?></td>
                        <td><?= htmlspecialchars($row['lokasi_awal']); ?></td>
...
function loadData(page = 1) {
    ...
    fetch('index_admin_utama.php?page_admin_utama=data_transaksi/mutasi/data_mutasi', {
        method: 'POST',
...
function downloadPDF() {
    ...
    notification.style.cssText = `
        ...
        font-weight: 500;
    `;
    notification.innerHTML = '<i class="fas fa-download"></i> Mengunduh PDF...';
    ...
    // Open PDF in new tab
    const url = 'index_admin_utama.php?page_admin_utama=data_transaksi/mutasi/cetak_mutasi' +
...
function showAddForm() {
    window.location.href = 'index_admin_utama.php?page_admin_utama=data_transaksi/mutasi/tambah_mutasi';
}
