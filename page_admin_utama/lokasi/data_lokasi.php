...
if(isset($_POST['ajax'])) {
    ...
    $where = "WHERE 1=1";
    if(!empty($search)) {
        $where .= " AND (nama_lokasi LIKE '%$search%' OR kode_lokasi LIKE '%$search%')";
    }

    // hitung total data
    $total_records_query = "SELECT COUNT(*) as count FROM lokasi $where";
    $total_result = mysqli_query($koneksi, $total_records_query);
    $total_records = mysqli_fetch_assoc($total_result)['count'];
    $total_pages = ceil($total_records / $limit);

    // ambil data lokasi
    $query = "SELECT * FROM lokasi $where ORDER BY kode_lokasi ASC LIMIT $start, $limit";
                ...
                <tr>
                    ...
                    <td>
                        <div class="action-buttons">
                            <a href="index_admin_utama.php?page_admin_utama=lokasi/edit_lokasi&kode_lokasi=<?= urlencode($row['kode_lokasi']); ?>" class="btn-edit">Edit</a>
                            <a href="index_admin_utama.php?page_admin_utama=lokasi/hapus_lokasi&kode_lokasi=<?= urlencode($row['kode_lokasi']); ?>" 
                               class="btn-delete" 
...
<div class="data-container">
    <div class="data-header">
        ...
        <div class="search-container">
            <input type="text" id="searchInput" class="search-input" placeholder="Cari berdasarkan nama/kode lokasi...">
        </div>
    </div>
...
function loadData(page = 1) {
    ...
    fetch('index_admin_utama.php?page_admin_utama=lokasi/data_lokasi', {
        method: 'POST',
...
function showAddForm() {
    window.location.href = 'index_admin_utama.php?page_admin_utama=lokasi/tambah_lokasi';
}
