...
<div style="text-align:center; font-family:Arial, sans-serif; margin-bottom:25px;"> 
  ...
  <h3 style="margin:8px 0; color:#333; font-size:16px;">LAPORAN DATA KERUSAKAN</h3>
...
<div style="padding-top:5px; font-size:10px; text-align:center; color:#555; font-family:Arial, sans-serif;">
  Halaman {PAGENO} dari {nbpg} | Laporan Kerusakan - '.$lokasi_text.' | '.$periode_text.'
</div>';
...
<h3>Data Kerusakan - '.$lokasi_text.' ('.$periode_text.')</h3>
<table class="table">
  <tr>
    <th width="4%">No</th>
...
if ($total_kerusakan > 0) {
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        <tr>
            <td>'.$no++.'</td>
            <td class="text-left">'.htmlspecialchars($row['no_kerusakan']).'</td>
