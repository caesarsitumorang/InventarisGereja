...
<div class="container">
  <div class="top-action-bar">
    <div>
      ...
      <p>Lihat dan kelola semua pelanggan di sistem.</p>
    </div>
  </div>
    ...
    <div class="scroll-horizontal">
      <table class="table-buku" id="bukuTable">
        ...
        <tbody>
          ...
          $no = 1;
          $query = "SELECT * FROM pelanggan ORDER BY nama_lengkap ASC";
          $sql = mysqli_query($koneksi, $query);

            if ($sql && mysqli_num_rows($sql) > 0) {
          while ($row = mysqli_fetch_assoc($sql)) {
            // kalau tidak ada foto, tampilkan default
            $foto = !empty($row['foto']) && file_exists("upload/".$row['foto']) 
                    ? $row['foto'] 
                    : "default.jpg";
            ...
            echo "<tr>
              <td>{$no}</td>
                 <td>
                <img src='upload/{$foto}' alt='Foto Pelanggan' style='width:50px; height:50px; object-fit:cover; border-radius:50%; border:2px solid #ddd;'>
              </td>
                <td>{$row['username']}</td>
                ...
                <td>{$row['alamat']}</td>
                <td class='aksi-btn'>
                  <a href='index_admin.php?page_admin=pelanggan/hapus_pelanggan&id={$row['id_pelanggan']}' class='btn-hapus' onclick='return confirm(\"Yakin ingin menghapus pelanggan ini?\")'>Hapus</a>
                </td>
              </tr>";
              $no++;
            }
          } else {
            echo "<tr><td colspan='9' style='text-align:center; color:#666;'>Tidak ada data pelanggan.</td></tr>";
          }
          ?>
        </tbody>
      </table>
